...
import daos.{AccountDAO, AdDAO, AnswerDAO, QuestionDAO}
import models.{Ad, Answer, Question}
...
class AdController @Inject()(accountDAO: AccountDAO(adDAO: AdDAO)(questionDAO: QuestionDAO)(answerDAO: AnswerDAO)
  extends Controller {
          ...
          case Some(question) => answerDAO.findAnswersForQuestion(question.id.get) flatMap {
            answers => if(checkAnswers(answers, answerRequest.answerIds)) {
              adDAO.findById(adId) map {
                ...
                case Some(ad) => Ok(Json.toJson(okJsonAnswer(ad)))
              }
            } else {
